...
public static class M10ToM6BRankDrivenHandoffRegression
{
    public static async Task<int> RunAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = await new MotorPipelineService().RunAsync(context, players, cancellationToken, "offline-c11-handoff");
            var competition = result.M10.FormationCompetition ?? [];
            var legal = result.M4.Candidates.Select(x => x.Formation)
                .Where(x => !string.IsNullOrWhiteSpace(x))
                .Distinct(StringComparer.Ordinal)
                .ToList();

            Check(competition.Count == legal.Count, "M10 competition does not cover every legal formation");
            ...
            Check(result.M6BFormationBudgets.Count == competition.Count, "M6-B budget map depth differs from M10 competition depth");
            Check(result.M6BFormationBudgets.Keys.OrderBy(x => x, StringComparer.Ordinal).SequenceEqual(competition.Select(x => x.Formation).OrderBy(x => x, StringComparer.Ordinal)), "M10 formations are not all handed off to M6-B");

            var expected = MotorPipelineService.BuildM6BFormationBudgetsForAcceptance(competition, 6, 3);
            foreach (var item in competition)
            {
                Check(result.M6BFormationBudgets.TryGetValue(item.Formation, out var actual), $"missing M6-B budget for {item.Formation}");
                var expectedBudget = expected[item.Formation];
                Check(actual!.BeamWidth == expectedBudget.BeamWidth, $"M6-B beam width mismatch for {item.Formation}");
                Check(actual.MaxIterations == expectedBudget.MaxIterations, $"M6-B iteration budget mismatch for {item.Formation}");
                Check(actual.BeamWidth >= 1 && actual.MaxIterations >= 1, $"invalid M6-B budget for {item.Formation}");
            }

            var rankOrder = competition.OrderBy(x => x.Rank).ToList();
            for (var i = 0; i < rankOrder.Count - 1; i++)
            {
                var a = result.M6BFormationBudgets[rankOrder[i].Formation];
                var b = result.M6BFormationBudgets[rankOrder[i + 1].Formation];
                Check(a.BeamWidth >= b.BeamWidth, "better M10 rank received a weaker M6-B beam budget");
                Check(a.MaxIterations >= b.MaxIterations, "better M10 rank received a weaker M6-B iteration budget");
            }

            var log = MotorRunLogStore.Get("offline-c11-handoff");
            Check(log is not null, "C11 run telemetry exists");
            var m10 = log!.Stages.FirstOrDefault(x => x.Motor == "M10" && x.Status == "completed");
            var m6b = log.Stages.FirstOrDefault(x => x.Motor == "M6-B" && x.Status == "completed");
            Check(m10 is not null, "M10 completion telemetry missing");
            Check(m6b is not null, "M6-B completion telemetry missing");
            Check(log.Stages.FindIndex(x => x.Motor == "M10" && x.Status == "completed") < log.Stages.FindIndex(x => x.Motor == "M6-B" && x.Status == "completed"), "M6-B completed before M10 handoff");

            Console.WriteLine($"M10 formations={competition.Count} | M6-B budgets={result.M6BFormationBudgets.Count} | winner={result.M10.BestPlan.Formation}");
            Console.WriteLine(string.Join(" | ", rankOrder.Select(x => $"#{x.Rank} {x.Formation}:B{result.M6BFormationBudgets[x.Formation].BeamWidth}/I{result.M6BFormationBudgets[x.Formation].MaxIterations}")));
            Console.WriteLine("PASS: C11 M10 -> M6-B rank-driven handoff");
            return 0;
        }
        catch (Exception ex) { return Fail("C11 exception: " + ex.Message); }
    }
