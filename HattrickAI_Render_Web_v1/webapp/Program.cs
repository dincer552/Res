...
app.Use(async(context,next)=>{try{await next();}catch(Exception ex){Console.Error.WriteLine($"UNHANDLED REQUEST ERROR: {ex}");if(!context.Response.HasStarted){context.Response.StatusCode=500;context.Response.ContentType="application/json; charset=utf-8";await context.Response.WriteAsJsonAsync(new{message=$"Sunucu hatası: {ex.Message}",errorType=ex.GetType().Name,chppTrace=ChppRequestTrace.Current?.ToResponse()});}}});
...
app.MapGet("/api/fixture-view/{matchId:int}",async(int matchId,int? recentIndex,ChppOAuthClient oauth)=>
{
    ...
    var fixtures=await matchService.LoadUpcomingFixturesAsync(own.TeamId);var fixture=fixtures.FirstOrDefault(x=>x.MatchId==matchId);if(fixture is null)return Results.NotFound(new{message="Maç bulunamadı.",chppTrace=trace.ToResponse()});
    ...
    if(!opponentRatingAvailable)return Results.BadRequest(new{message=$"Rakibin geçmiş kadrosu için aynı rating motoru çalıştırılamadı: {ratingError}",ratingSource="CHPP_PLAYER_SKILLS_INCOMPLETE",chppTrace=trace.ToResponse()});
