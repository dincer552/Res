...
app.MapGet("/api/fixture-view/{matchId:int}",async(int matchId,int? recentIndex,ChppOAuthClient oauth)=>
{
    ...
    if(selectedHistory is null)return Results.BadRequest(new{message="Rakibin seçilmiş geçmiş maçı bulunamadı; aynı rating motoruyla hesaplama yapılamıyor.",ratingSource="CHPP_HISTORY_UNAVAILABLE"});
    var historicalPlayers=await lineupService.LoadAsync(selectedHistory.Fixture.MatchId,selected.OpponentTeamId);
    if(historicalPlayers.Count!=11)return Results.BadRequest(new{message="Rakibin geçmiş maç kadrosu CHPP'den 11 oyuncu olarak alınamadı; sahte rating kullanılmayacak.",ratingSource="CHPP_MATCH_LINEUP_INCOMPLETE",playerCount=historicalPlayers.Count});
    var opponentFormation=InferFormation(historicalPlayers);var historicalSlots=BuildHistoricalSlots(historicalPlayers,opponentFormation);var symmetricRatingEngine=new SymmetricMatchRatingEngine();
    bool opponentRatingAvailable=symmetricRatingEngine.TryCalculateHistorical(opponent.Players,historicalSlots,opponentFormation,new TeamMatchContext{IsHome=selectedHistory.Fixture.HomeTeamId==selected.OpponentTeamId,TacticType=selectedHistory.OpponentTeam.TacticType,TacticLevel=selectedHistory.OpponentTeam.TacticLevel,SlotBehaviours=historicalSlots.Select((s,i)=>(i,s.Behaviour)).ToDictionary(x=>x.i,x=>x.s)},out var calculatedOpponentRatings,out var calculatedOpponentLineup,out var calculatedOpponentBehaviours,out var ratingError);
    if(!opponentRatingAvailable)return Results.BadRequest(new{message=$"Rakibin geçmiş kadrosu için aynı rating motoru çalıştırılamadı: {ratingError}",ratingSource="CHPP_PLAYER_SKILLS_INCOMPLETE"});
    var simulationOpponent=new TeamData(selectedHistory.OpponentTeam.TeamName,calculatedOpponentRatings,selectedHistory.OpponentTeam.TacticType,selectedHistory.OpponentTeam.TacticLevel);
    ...
    var opponentLineupView=BuildCalculatedHistoricalLineupView(calculatedOpponentLineup,opponentFormation,calculatedOpponentBehaviours,calculatedOpponentRatings,historicalPlayers);
    return Results.Ok(new{fixture,isHome,selectedRecentIndex=historyIndex,selectedOpponentMatch=new{selectedHistory.Fixture,selectedHistory.OpponentTeam.TeamName,selectedHistory.OpponentTeam.TacticType,selectedHistory.OpponentTeam.TacticLevel,actualMatchRatings=selectedHistory.OpponentTeam.Ratings,calculatedRatings=calculatedOpponentRatings,ratingSource="CHPP_PLAYER_SKILLS_SAME_ENGINE"},ownTeam=new{teamId=own.TeamId,teamName=own.TeamName},opponentTeam=new{teamId=opponent.TeamId,teamName=opponent.TeamName},ownLineup=BuildLineupView(recommendation.Lineup,recommendation.Formation,recommendation.BehaviourProfile,recommendation.Ratings),opponentLineup=opponentLineupView,ownRatings=recommendation.Ratings,opponentRatings=calculatedOpponentRatings,formation=recommendation.Formation,tactic=new{recommendation.TacticName,recommendation.TacticType,recommendation.TacticLevel},recommendation=new{recommendation.Explanation,recommendation.SelectionScore},recentMatches=selected.RecentMatches.Select(m=>new{m.Fixture,opponent=new{m.OpponentTeam.TeamName,actualMatchRatings=m.OpponentTeam.Ratings,m.OpponentTeam.TacticType,m.OpponentTeam.TacticLevel}})});
});
...
static object BuildCalculatedHistoricalLineupView(List<PlayerData> lineup,string formation,IReadOnlyDictionary<int,PlayerBehaviour> behaviours,TeamRatings calculatedRatings,IReadOnlyList<ChppLineupPlayer> historicalPlayers)
{
    var roles=LineupRatingEngine.GetRoles(formation);var ratingEngine=new LineupRatingEngine();var historicalById=historicalPlayers.ToDictionary(p=>p.PlayerId);
