...
app.MapGet("/api/fixture-view/{matchId:int}",async(int matchId,int? recentIndex,ChppOAuthClient oauth)=>
{
    ...
    if(string.IsNullOrWhiteSpace(opponentFormation))
        return Results.BadRequest(new{message="Rakibin geçmiş maç formasyonu CHPP kadrosundan çözülemedi.",ratingSource="CHPP_FORMATION_UNAVAILABLE"});
    ...
    return Results.Ok(new{
        ...
        selectedOpponentMatch=new{selectedHistory.Fixture,selectedHistory.OpponentTeam.TeamName,selectedHistory.OpponentTeam.TacticType,selectedHistory.OpponentTeam.TacticLevel,actualMatchRatings=selectedHistory.OpponentTeam.Ratings,calculatedRatings=calculatedOpponentRatings,ratingSource="CHPP_PLAYER_SKILLS_SAME_ENGINE"},
        ...
        opponentLineup=opponentLineupView,ownRatings=recommendation.Ratings,opponentRatings=calculatedOpponentRatings,formation=recommendation.Formation,
        tactic=new{recommendation.TacticName,recommendation.TacticType,recommendation.TacticLevel},recommendation=new{recommendation.Explanation,recommendation.SelectionScore},
