(()=>{
  ...
  function ensureControls(){
    ...
    if(!document.getElementById('ownFormationWrap')){
      const old=document.getElementById('ownFormation');
      if(!old)return;
      const current=old.textContent?.trim();
      if(!selectedFormation&&FORMATIONS.includes(current))selectedFormation=current;
      const wrap=document.createElement('div');
      wrap.id='ownFormationWrap';wrap.className='own-formation-wrap';
      ...
      const btn=wrap.querySelector('#ownFormation');
      btn.addEventListener('click',()=>toggleFormationMenu());
  ...
  function toggleFormationMenu(){
    const menu=document.getElementById('ownFormationMenu'),btn=document.getElementById('ownFormation');if(!menu||!btn)return;
  ...
  function updateFormationChecks(){
    document.querySelectorAll('.own-formation-option').forEach(b=>{
      const selected=b.dataset.formation===selectedFormation;
      b.classList.toggle('selected',selected);
      const check=b.querySelector('.own-formation-check');if(check)check.textContent=selected?'✓':'';
    });
    const btn=document.getElementById('ownFormation');if(btn)btn.textContent=selectedFormation||'—';
  }

  async function getOwnTeam(){
    if(ownTeamPromise)return ownTeamPromise;
    ownTeamPromise=fetch('/api/team').then(jsonResponse).then(x=>{
      if(!x?.players?.length)throw new Error('Takım kadrosu alınamadı.');
      return x;
    }).catch(e=>{ownTeamPromise=null;throw e;});
  ...
  async function runFormationAnalysis(){
    ...
    formationBusy=true;
    const btn=document.getElementById('ownFormation');
    const status=document.getElementById('ownFormationStatus');
    if(btn){btn.classList.add('loading');btn.textContent='…';}
    if(status)status.textContent=`${selectedFormation} için HO Engine düşünüyor…`;
    try{
      ...
      const opponent={
        ...
        tacticType:Number(currentView.tactic?.tacticType??0),
        tacticLevel:Number(currentView.tactic?.tacticLevel??0),
      ...
      if(!r.ok)throw new Error(x.message||'Seçilen diziliş için kadro oluşturulamadı.');
      currentView.__selectedFormationResult=x;
      currentView.ownRatings=x.ratings||currentView.ownRatings;
      currentView.ownLineup={formation:x.formation,ratings:x.ratings,playerCount:x.lineup?.length||0,players:(x.lineup||[]).map(p=>({name:p.name,role:roleText(p.roleKey),roleKey:p.roleKey,rating:p.rating,behaviour:p.behaviour,form:p.form,stamina:p.stamina}))};
      ...
      renderSelectedFormationResult(x);
      if(status)status.textContent=`HO Engine • ${x.formation} • ${x.tacticName||'Normal'}`;
    }catch(e){
      if(status)status.textContent=e.message||'Diziliş hesaplanamadı.';
    }finally{
      formationBusy=false;
      if(btn){btn.classList.remove('loading');btn.textContent=selectedFormation||'—';}
      updateFormationChecks();
    }
  }
  ...
  function renderSelectedFormationResult(x){
    ...
    const title=document.getElementById('ownLineupTitle');if(title)title.textContent='En iyi 11';
    ...
    if(typeof ratingForm==='function'&&currentView){
      const home=currentView.isHome?(x.ratings||{}):(currentView.opponentRatings||{});
      const away=currentView.isHome?(currentView.opponentRatings||{}):(x.ratings||{});
      ratingForm('#homeRatings','home',home);ratingForm('#awayRatings','away',away);
    }
  }
  ...
  function applyOwnMode(){
    ...
    if(selectedMode==='cup'&&!cupReady){selectedMode='best';}
    document.querySelectorAll('#ownLineupMode button').forEach(b=>b.classList.toggle('active',b.dataset.mode===selectedMode));

    if(selectedMode==='best'&&currentView?.__selectedFormationResult){
      const x=currentView.__selectedFormationResult;
    ...
    if(source){
      ...
      }else source.textContent=`Lig analizi • HO Engine en iyi 11${formation&&formation!=='—'?` • ${formation}`:''}`;
    }
    if(currentView){
  ...
  function patchRender(){
    ...
    if(old.__formationPatched)return true;
    const wrapped=function(x){old(x);ensureControls();selectedFormation=x?.ownLineup?.formation||selectedFormation;updateFormationChecks();applyOwnMode();decorateOpponentSource();};
    wrapped.__formationPatched=true;
  ...
  async function loadCup(){
    try{const r=await fetch('/api/cup-lineup/latest');const x=await jsonResponse(r);if(!r.ok)throw new Error(x.message||'Kupa kadrosu alınamadı.');latestCup=x;ensureControls();if(!selectedFormation)selectedFormation=x.record?.formation||null;applyOwnMode();}
  ...
  const start=()=>{
    ...
    patchRender();
    loadCup();
    setTimeout(()=>{ensureControls();patchRender();decorateOpponentSource();applyOwnMode();},250);
  };
