...
public sealed class OfflineExportService
{
    ...
    public async Task<object> ExportAsync(string build, MatchQuestionnaire questionnaire, CancellationToken ct)
    {
        ...
        // Önce mevcut V5 analizini çalıştır: export ile ekranda kullanılan veri aynı kalır.
        ...
        var ownProfiles = _playerAnalysis.AnalyzeAll(ownPlayers);
        ...
        // Bazı CHPP izinlerinde rakip oyuncu havuzu erişilebilir olmayabilir. Export yine de devam eder.
        string? opponentPlayersXml = null;
        string? opponentPlayersError = null;
        try
        {
            opponentPlayersXml = await _chpp.GetXmlAsync("players", new Dictionary<string,string?> { ["version"] = "1.3", ["teamId"] = opponentId.ToString(CultureInfo.InvariantCulture) }, ct);
        }
        catch (Exception ex)
        {
            opponentPlayersError = ex.Message;
        }

        return new
        {
            schema = "hattrickai-v5-offline-test-v2",
            ...
            {
                ownTeamId,
                ownTeam = analysis.TeamName,
                opponentTeamId = opponentId,
                opponentTeam = analysis.OpponentName,
            ...
            {
                teamDetails = teamXml,
                training = trainingXml,
                ownPlayers = ownPlayersXml,
                ownMatches = matchesXml,
                opponentMatches = opponentMatchesXml,
                opponentLastLineup = lineupXml,
                opponentLastMatchDetails = matchDetailsXml,
                opponentPlayers = opponentPlayersXml,
                opponentPlayersError
            },
            normalized = new
            {
                ownPlayers,
                ownPlayerAnalysis = ownProfiles,
                opponentLastLineup = ParseLineup(lineupXml),
                opponentLastMatchRatings = ParseMatchRatings(matchDetailsXml, opponentId)
            },
            v5Analysis = analysis
        };
    }
    ...
    private static List<Player> ReadPlayers(string xml)
        => XmlV5.Root(xml)?.Descendants("Player")
            .Select(p => new Player(
                ...
                XmlV5.Int(p,"StaminaSkill"), XmlV5.Int(p,"PlayerForm"), XmlV5.Int(p,"Experience"), XmlV5.Int(p,"Loyalty")))
    ...
    private static List<(int MatchId,DateTimeOffset Date,int HomeId,int AwayId,string HomeName,string AwayName,int MatchType)> ReadMatches(string xml, int teamId)
    {
        ...
        foreach (var m in XmlV5.Root(xml)?.Descendants("Match") ?? Enumerable.Empty<XElement>())
        {
            ...
            var home = XmlV5.Int(m,"HomeTeamID"); var away = XmlV5.Int(m,"AwayTeamID"); var type = XmlV5.Int(m,"MatchType");
    ...
    private static object ParseLineup(string xml)
    {
        ...
        var players = team?.Element("Lineup")?.Elements("Player").Select(p => new
        {
            playerId = XmlV5.Int(p,"PlayerID"),
            playerName = XmlV5.Text(p,"PlayerName"),
            positionCode = XmlV5.Int(p,"PositionCode"),
            roleId = XmlV5.Int(p,"RoleID"),
            behaviour = XmlV5.Int(p,"Behaviour"),
            ratingStars = XmlV5.Text(p,"RatingStars")
        }).Where(x => x.playerId > 0).ToList() ?? new();
    ...
    private static object ParseMatchRatings(string xml, int opponentId)
    {
        ...
        {
            available = true,
            leftDefence = XmlV5.Int(team,"RatingLeftDef") / 4.0,
            centralDefence = XmlV5.Int(team,"RatingMidDef") / 4.0,
            rightDefence = XmlV5.Int(team,"RatingRightDef") / 4.0,
            midfield = XmlV5.Int(team,"RatingMidfield") / 4.0,
            leftAttack = XmlV5.Int(team,"RatingLeftAtt") / 4.0,
            centralAttack = XmlV5.Int(team,"RatingMidAtt") / 4.0,
            rightAttack = XmlV5.Int(team,"RatingRightAtt") / 4.0
        };
    }
