...
async function loadJs(){loadCss();if(!window.__integratedPlanningWizardReady)await loadScript('integratedPlanningWizardJs',`${ROOT}integrated-planning-wizard.js?v=20260822-wizard-01`);if(!document.getElementById('integratedPlanningWizardGuardJs'))await loadScript('integratedPlanningWizardGuardJs',`${ROOT}integrated-planning-wizard-guard.js?v=20260822-wizard-01`)}
async function openWizard(){try{await loadJs();if(typeof window.openIntegratedPlanningWizard==='function')window.openIntegratedPlanningWizard();else throw new Error('Kadro planı arayüzü hazır değil.')}catch(e){console.error(e);alert(e.message||'Kadro planı başlatılamadı.')}}
