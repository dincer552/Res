...
{
    private const int DefaultCalibrationLimit = 260;
    private const int MaxCalibrationLimit = 300;
    private const int ArchiveWindowDays = 45;
    private const int ChppDelayMilliseconds = 5000;
    ...
    public async Task<object> GetAsync(CancellationToken ct)
    {
        ...
        var upcomingLeague = ownMatches.Where(m => m.Date > now && m.MatchType == 1).OrderBy(m => m.Date).Select(m => new
        {
            matchId = m.MatchId, date = m.Date, homeTeam = m.HomeTeam, homeTeamId = m.HomeTeamId,
            awayTeam = m.AwayTeam, awayTeamId = m.AwayTeamId, matchType = m.MatchType,
            matchTypeName = m.MatchTypeName, isHome = m.HomeTeamId == teamId,
        ...
        {
            m.matchId, m.date, m.homeTeam, m.homeTeamId, m.awayTeam, m.awayTeamId, m.matchType,
            m.matchTypeName, m.isHome, m.opponentTeam, m.opponentTeamId,
        ...
        return new
        {
            matchId = last.MatchId, date = last.Date, homeTeam = last.HomeTeam, awayTeam = last.AwayTeam,
            homeGoals = last.HomeGoals, awayGoals = last.AwayGoals, matchType = last.MatchType,
            matchTypeName = last.MatchTypeName, opponentTeam = opponentId == last.HomeTeamId ? last.HomeTeam : last.AwayTeam,
            opponentWasHome = opponentId == last.HomeTeamId, finished = last.HomeGoals.HasValue && last.AwayGoals.HasValue,
            ownLogoUrl, upcomingMatches = upcomingWithLogos, opponentLogoUrl = logoCache.GetValueOrDefault(opponentId, string.Empty)
        };
    }
    ...
    private async Task<object> CollectCalibration(CancellationToken ct)
    {
        ...
        var requested = int.TryParse(_http.HttpContext?.Request.Query["limit"], out var n) ? n : DefaultCalibrationLimit;
        var limit = Math.Clamp(requested, 10, MaxCalibrationLimit);
        ...
        var eligible = archiveMatches.Where(m => m.Date < DateTimeOffset.UtcNow && m.HomeGoals.HasValue && m.AwayGoals.HasValue && IsCalibrationMatchType(m.MatchType)).OrderByDescending(m => m.Date).Take(limit).ToList();
        ...
        foreach (var match in eligible)
        {
            if (previousDetail is not null)
                await Task.Delay(ChppDelayMilliseconds, ct);
            try
            {
                ...
                previousDetail = match.MatchId.ToString(CultureInfo.InvariantCulture);
                ...
                rows.Add(new
                {
                    matchId = match.MatchId, date = match.Date, isHome = ownHome,
                    ownPossessionPercent = Math.Round(ownPoss, 2),
                    homePossessionFirstHalf = hp, awayPossessionFirstHalf = ap,
                    homePossessionSecondHalf = hp2, awayPossessionSecondHalf = ap2,
                    homeSectorChances = homeSector, awaySectorChances = awaySector,
                    ownSectorChances = ownSector, opponentSectorChances = opponentSector,
                    homeLeftChances = homeLeft, homeCentreChances = homeCenter, homeRightChances = homeRight,
                    awayLeftChances = awayLeft, awayCentreChances = awayCenter, awayRightChances = awayRight,
                    ...
                    homeTacticSkill = XmlV5.Int(home, "TacticSkill"), awayTacticSkill = XmlV5.Int(away, "TacticSkill"),
                    homeRatingMidfield = XmlV5.Double(home, "RatingMidfield"), awayRatingMidfield = XmlV5.Double(away, "RatingMidfield"),
                    homeRatingLeftDef = XmlV5.Double(home, "RatingLeftDef"), homeRatingMidDef = XmlV5.Double(home, "RatingMidDef"), homeRatingRightDef = XmlV5.Double(home, "RatingRightDef"),
                    homeRatingLeftAtt = XmlV5.Double(home, "RatingLeftAtt"), homeRatingMidAtt = XmlV5.Double(home, "RatingMidAtt"), homeRatingRightAtt = XmlV5.Double(home, "RatingRightAtt"),
                    awayRatingLeftDef = XmlV5.Double(away, "RatingLeftDef"), awayRatingMidDef = XmlV5.Double(away, "RatingMidDef"), awayRatingRightDef = XmlV5.Double(away, "RatingRightDef"),
                    awayRatingLeftAtt = XmlV5.Double(away, "RatingLeftAtt"), awayRatingMidAtt = XmlV5.Double(away, "RatingMidAtt"), awayRatingRightAtt = XmlV5.Double(away, "RatingRightAtt")
                });
            }
            catch (Exception ex)
            {
        ...
        return new
        {
            ok = true, phase = "D", schema = "hattrickai-v5-historical-production-v1",
            source = "CHPP", requestedLimit = limit, minimumAcceptanceMatches = 250,
            archiveWindowDays = ArchiveWindowDays, archiveWindowCount = CountArchiveWindows(startDate, endDate),
            archiveRawMatchCount = archiveMatches.Count, archiveUniqueMatchCount = archiveMatches.Select(x => x.MatchId).Distinct().Count(),
            sampleCount = eligible.Count, detailsFetched, failedDetails, chanceSamples,
            ...
            notes = "Historical CHPP observation corpus. Matchdetails are fetched sequentially with a 5-second delay to avoid burst traffic. No production coefficient is changed by collection.",
            rows
        };
    }
    ...
    private async Task<List<ReferenceMatch>> ReadArchiveAcrossWindows(int teamId, DateTimeOffset startDate, DateTimeOffset endDate, CancellationToken ct)
    {
        ...
        while (cursorEnd >= startDate)
        {
            ...
            all.AddRange(await ReadArchiveWindow(teamId, cursorStart, cursorEnd, ct));
            if (cursorStart <= startDate) break;
            cursorEnd = cursorStart.AddDays(-1);
        }
        return all.GroupBy(x => x.MatchId).Select(g => g.OrderByDescending(x => x.Date).First()).OrderByDescending(x => x.Date).ToList();
    }
    ...
    private async Task<List<ReferenceMatch>> ReadArchiveWindow(int teamId, DateTimeOffset first, DateTimeOffset last, CancellationToken ct)
    {
        ...
        {
            ["version"] = "1.5", ["teamID"] = teamId.ToString(CultureInfo.InvariantCulture),
        ...
        foreach (var m in XmlV5.Root(xml)?.Descendants("Match") ?? Enumerable.Empty<XElement>())
        {
            ...
            result.Add(new ReferenceMatch(id, date, XmlV5.Text(m, "HomeTeamName"), home, XmlV5.Text(m, "AwayTeamName"), away,
                NullableInt(m, "HomeGoals"), NullableInt(m, "AwayGoals"), type, MatchTypeName(type)));
        }
        return result;
    }
    ...
    private async Task<string> ReadLogoUrl(int teamId, CancellationToken ct)
    {
        ...
        try
        {
            var xml = await _chpp.GetXmlAsync("teamdetails", new Dictionary<string, string?> { ["version"] = "3.0", ["teamID"] = teamId.ToString(CultureInfo.InvariantCulture) }, ct);
    ...
    {
        var xml = await _chpp.GetXmlAsync("matches", new Dictionary<string, string?> { ["version"] = "2.2", ["teamID"] = teamId.ToString(CultureInfo.InvariantCulture) }, ct);
        return ParseMatches(xml);
    }

    private static List<ReferenceMatch> ParseMatches(string xml)
    {
        var result = new List<ReferenceMatch>();
        foreach (var m in XmlV5.Root(xml)?.Descendants("Match") ?? Enumerable.Empty<XElement>())
        {
    ...
    private static int? NullableInt(XElement parent, string name) => int.TryParse(XmlV5.Text(parent, name), NumberStyles.Integer, CultureInfo.InvariantCulture, out var value) ? value : null;
    private static int CountArchiveWindows(DateTimeOffset startDate, DateTimeOffset endDate) => (int)Math.Ceiling((endDate.Date - startDate.Date).TotalDays / ArchiveWindowDays);
    ...
    {
        1 => "Lig", 2 => "Play-off", 3 => "Kupa", 4 => "Hazırlık", 5 => "Kupa hazırlık", 7 => "Hattrick Masters",
        8 => "Uluslararası hazırlık", 9 => "Uluslararası kupa hazırlık", 10 => "Milli takım", 11 => "Milli takım kupa", 12 => "Milli takım hazırlık", _ => "Maç"
    };

    private sealed record ReferenceMatch(int MatchId, DateTimeOffset Date, string HomeTeam, int HomeTeamId, string AwayTeam, int AwayTeamId,
        int? HomeGoals, int? AwayGoals, int MatchType, string MatchTypeName);
}
