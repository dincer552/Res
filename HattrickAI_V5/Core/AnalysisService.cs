...
public sealed class AnalysisService
{
    ...
    private static Slot HistoricalSlot(XElement node,RegionalRatingSnapshot teamRating,int experienceLevel)
    {
        ...
        return new Slot(map.Item1,map.Item2,map.Item2,name,id,rating,map.Item3,map.Item4,order,stars);
    }
    ...
    private static Lineup Formation(IReadOnlyList<Slot> slots){var d=slots.Count(x=>x.Code.StartsWith("DEF",StringComparison.Ordinal));var m=slots.Count(x=>x.Code.StartsWith("IM",StringComparison.Ordinal)||x.Code.StartsWith("W-",StringComparison.Ordinal));var f=slots.Count(x=>x.Code.StartsWith("FW",StringComparison.Ordinal));return new Lineup("Opponent",$"{d}-{m}-{f}",slots);}

    private static Lineup BuildOwnLineup(string teamName,List<Player> players)
    {
        var unused=new HashSet<int>(players.Select(p=>p.Id));Player Pick(Func<Player,double> score){var p=players.Where(p=>unused.Contains(p.Id)).OrderByDescending(score).FirstOrDefault();if(p is null)throw new InvalidOperationException("Önerilen 11 oluşturulamadı.");unused.Remove(p.Id);return p;}
        var gk=Pick(p=>p.Keeper+p.Form*.15);var dl=Pick(p=>p.Defending+p.Passing*.10+p.Winger*.05);var dc=Pick(p=>p.Defending*1.05+p.Passing*.15+p.Playmaking*.04);var dr=Pick(p=>p.Defending+p.Passing*.10+p.Winger*.05);var wl=Pick(p=>p.Winger+p.Passing*.22+p.Playmaking*.08);var il=Pick(p=>p.Playmaking+p.Passing*.25+p.Stamina*.12);var ic=Pick(p=>p.Playmaking*1.05+p.Passing*.25+p.Stamina*.12+p.Experience*.04);var ir=Pick(p=>p.Playmaking+p.Passing*.25+p.Stamina*.12);var wr=Pick(p=>p.Winger+p.Passing*.22+p.Playmaking*.08);var fl=Pick(p=>p.Scoring+p.Passing*.18+p.Winger*.08);var fr=Pick(p=>p.Scoring*1.05+p.Passing*.18+p.Winger*.08+p.Experience*.04);
        var slots=new List<Slot>{MakeSlot("GK","GK","Kaleci",gk,50,10),MakeSlot("DEF-CL","DEF-CL","Sol stoper",dl,30,34),MakeSlot("DEF-C","DEF-C","Merkez stoper",dc,50,34),MakeSlot("DEF-CR","DEF-CR","Sağ stoper",dr,70,34),MakeSlot("W-L","W-L","Sol kanat",wl,12,50),MakeSlot("IM-L","IM-L","Sol iç",il,34,50),MakeSlot("IM-C","IM-C","Merkez",ic,50,50),MakeSlot("IM-R","IM-R","Sağ iç",ir,66,50),MakeSlot("W-R","W-R","Sağ kanat",wr,88,50),MakeSlot("FW-L","FW-L","Sol forvet",fl,38,72),MakeSlot("FW-R","FW-R","Sağ forvet",fr,62,72)};
    ...
    {
        var rating=code=="GK"?p.Keeper*.75+p.Defending*.15+p.Form*.10:code.StartsWith("DEF")?p.Defending*.70+p.Passing*.15+p.Playmaking*.10+p.Form*.05:code.StartsWith("IM")?p.Playmaking*.55+p.Passing*.20+p.Stamina*.15+p.Defending*.10:code.StartsWith("W-")?p.Winger*.45+p.Playmaking*.20+p.Passing*.20+p.Defending*.10+p.Form*.05:p.Scoring*.55+p.Passing*.20+p.Winger*.15+p.Playmaking*.10;
