...
public static class FinalPredictionContinuityRegression
{
    public static async Task<int> RunAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = await new MotorPipelineService().RunAsync(context, players, cancellationToken, "offline-c17-finalprediction");
            ...
            var m11 = result.M11!.Prediction!;
            var m9 = result.M9!;
            var db2 = result.CandidateDatabase2;
            var winnerId = Signature(result.FinalPlan.Lineup);
            var winner = db2.FirstOrDefault(x => x.CandidateId == winnerId);
            Check(winner is not null, "FinalPlan winner is absent from DB2");
            Check(winner!.Prediction is not null, "DB2 winner prediction missing");

            Check(final == m11, "FinalPrediction is not identical to M11 prediction");
            Check(final == m9.Prediction, "FinalPrediction is not identical to selected M9 prediction");
            Check(final == winner.Prediction, "FinalPrediction is not identical to DB2 winner prediction");
            Check(final.CandidateId == winnerId, "FinalPrediction candidate identity differs from FinalPlan winner");
            Check(final.Formation == result.FinalPlan.Formation, "FinalPrediction formation differs from FinalPlan");
            Check(double.IsFinite(final.Prediction.ExpectedHomeGoals) && double.IsFinite(final.Prediction.ExpectedAwayGoals), "FinalPrediction expected goals are not finite");
            Check(double.IsFinite(final.Prediction.WinProbability) && double.IsFinite(final.Prediction.DrawProbability) && double.IsFinite(final.Prediction.LossProbability), "FinalPrediction W/D/L are not finite");
            Check(final.Prediction.WinProbability is >= 0 and <= 1 && final.Prediction.DrawProbability is >= 0 and <= 1 && final.Prediction.LossProbability is >= 0 and <= 1, "FinalPrediction W/D/L are out of bounds");
            Check(Math.Abs(final.Prediction.WinProbability + final.Prediction.DrawProbability + final.Prediction.LossProbability - 1.0) <= 1e-9, "FinalPrediction W/D/L do not sum to 1");
            ...
            Check(!string.IsNullOrWhiteSpace(final.MostLikelyScore), "FinalPrediction most-likely score missing");

            var log = MotorRunLogStore.Get("offline-c17-finalprediction");
            ...
            Console.WriteLine($"FinalPrediction={final.Formation} | W/D/L={final.Prediction.WinProbability:P1}/{final.Prediction.DrawProbability:P1}/{final.Prediction.LossProbability:P1} | score={final.MostLikelyScore}");
            Console.WriteLine("PASS: C17 FinalPrediction continuity");
            return 0;
        }
        catch (Exception ex) { return Fail("C17 exception: " + ex.Message); }
    }
