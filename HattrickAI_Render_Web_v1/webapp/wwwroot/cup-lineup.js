...
// CHPP matchLineup returns the exact historical XI with PositionCode/RoleID.
// Normalize it to the canonical lineup shape before renderPitch so repositioned
// players stay in the same real positions they occupied in that match.
function cupHistoricalPlayer(p){
  ...
  switch(pc){
    ...
    case 10:
    case 11:
      roleKey=roleId===10||roleId===111?'RightForward':roleId===11||roleId===113?'LeftForward':'CentralForward';
      role='SF'; break;
  }
  return {
...
function renderCupLineup(x){
  ...
  const cacheLabel=x.cache?.lineup||x.cache?.mode||'CHPP';
  document.getElementById('cupLineupStatus').textContent=`${r.teamName} • gerçek maç kadrosu • ${cacheLabel==='POSTGRES_CACHE'?'kayıtlı veriden':'CHPP’den alındı ve kaydedildi'}`;
...
async function loadCupLineup(){
  ...
  if(!status)return;
  status.textContent='Son kupa maçının gerçek kadrosu yükleniyor…';
  try{
    const r=await fetch('/api/cup-lineup/latest',{cache:'no-store'});
    const x=await jsonResponse(r);
    if(!r.ok)throw new Error(x.message||'Son kupa kadrosu alınamadı.');
    renderCupLineup(x);
  }catch(e){
    console.error('CUP LINEUP LOAD FAILED',e);
    status.textContent=e.message||'Son kupa kadrosu alınamadı.';
