...
public sealed class AnalysisService
{
    ...
    public async Task<Analysis> RunAsync(string build, MatchQuestionnaire? questionnaire, CancellationToken ct)
    {
        ...
        var next = matches
            .Where(x => x.Date > now && IsCompetitiveMatchType(x.MatchType))
            .OrderBy(x => x.Date)
            .FirstOrDefault();
        ...
        var experienceLevel = Math.Clamp(
            XmlV5.Int(lineupRoot?.Descendants("Team").FirstOrDefault(), "ExperienceLevel"), 1, 20);
        ...
        // M3 -> M10 is now the sole source of the recommended XI.
        // The previous direct XIOptimizationService path is intentionally gone.
    ...
    private static List<XElement> SelectFinalFieldPlayers(XElement? root)
    {
        ...
        var finalPlayers = team.Element("Lineup")?.Elements("Player")
            ...
            .ToList() ?? new();
        var starting = team.Element("StartingLineup")?.Elements("Player")
    ...
    private async Task<RegionalRatingSnapshot> ReadDirectHistoricalOpponentRating(int matchId, int opponentId, CancellationToken ct)
    {
        var xml = await _chpp.GetXmlAsync(
            "matchdetails",
            new Dictionary<string, string?> { ["version"] = "1.4", ["matchID"] = matchId.ToString(CultureInfo.InvariantCulture) },
            ct);
        var match = XmlV5.Root(xml)?.Descendants("Match").FirstOrDefault();
        if (XmlV5.Int(match, "MatchID") != matchId)
            throw new InvalidOperationException("CHPP matchdetails MatchID uyuşmuyor.");
        ...
        if (team is null || TeamNodeId(team) != opponentId)
            throw new InvalidOperationException("Rakibin matchdetails kaydı alınamadı.");

        return new RegionalRatingSnapshot(
            XmlV5.Int(team, "RatingLeftDef") / 4.0,
            XmlV5.Int(team, "RatingMidDef") / 4.0,
            XmlV5.Int(team, "RatingRightDef") / 4.0,
            XmlV5.Int(team, "RatingMidfield") / 4.0,
            XmlV5.Int(team, "RatingLeftAtt") / 4.0,
            XmlV5.Int(team, "RatingMidAtt") / 4.0,
    ...
    private async Task<List<Player>> ReadPlayers(int teamId, CancellationToken ct)
    {
        var xml = await _chpp.GetXmlAsync(
            ...
            ct);
        return XmlV5.Root(xml)?.Descendants("Player")
            .Select(p => new Player(
