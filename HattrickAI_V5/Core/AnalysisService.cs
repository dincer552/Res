...
public sealed class AnalysisService
{
    ...
    {
        var team = root?.Descendants("Team").FirstOrDefault(); if (team is null) return new();
        ...
        foreach (var s in team.Descendants("Substitution")) { var subject=XmlV5.Int(s,"SubjectPlayerID"); var obj=XmlV5.Int(s,"ObjectPlayerID"); if(subject>0&&obj>0&&onField.Remove(subject)) onField.Add(obj); }
    ...
    {
        var xml=await _chpp.GetXmlAsync("matchdetails",new Dictionary<string,string?>{["version"]="1.4",["matchID"]=matchId.ToString(CultureInfo.InvariantCulture)},ct); var match=XmlV5.Root(xml)?.Descendants("Match").FirstOrDefault();
        if(XmlV5.Int(match,"MatchID")!=matchId) throw new InvalidOperationException("CHPP matchdetails MatchID uyuşmuyor."); var team=match?.Element("HomeTeam"); if(team is null||TeamNodeId(team)!=opponentId)team=match?.Element("AwayTeam"); if(team is null||TeamNodeId(team)!=opponentId)throw new InvalidOperationException("Rakibin matchdetails kaydı alınamadı.");
        return new RegionalRatingSnapshot(XmlV5.Int(team,"RatingLeftDef")/4.0,XmlV5.Int(team,"RatingMidDef")/4.0,XmlV5.Int(team,"RatingRightDef")/4.0,XmlV5.Int(team,"RatingMidfield")/4.0,XmlV5.Int(team,"RatingLeftAtt")/4.0,XmlV5.Int(team,"RatingMidAtt")/4.0,XmlV5.Int(team,"RatingRightAtt")/4.0,XmlV5.Int(team,"RatingLeftDef")/4.0,XmlV5.Int(team,"RatingMidDef")/4.0,XmlV5.Int(team,"RatingRightDef")/4.0,XmlV5.Int(team,"RatingMidfield")/4.0,XmlV5.Int(team,"RatingLeftAtt")/4.0,XmlV5.Int(team,"RatingMidAtt")/4.0,XmlV5.Int(team,"RatingRightAtt")/4.0);
    }
    ...
    private async Task<List<Player>> ReadPlayers(int teamId,CancellationToken ct)
    {
        ...
        return XmlV5.Root(xml)?.Descendants("Player").Select(p=>new Player(XmlV5.Int(p,"PlayerID"),XmlV5.Text(p,"PlayerName"),XmlV5.Int(p,"KeeperSkill"),XmlV5.Int(p,"DefenderSkill"),XmlV5.Int(p,"PlaymakerSkill"),XmlV5.Int(p,"PassingSkill"),XmlV5.Int(p,"WingerSkill"),XmlV5.Int(p,"ScorerSkill"),XmlV5.Int(p,"StaminaSkill"),XmlV5.Int(p,"PlayerForm"),XmlV5.Int(p,"Experience"),XmlV5.Int(p,"Loyalty"),XmlV5.Int(p,"InjuryLevel"))).Where(p=>p.Id>0).ToList()??new();
    }
